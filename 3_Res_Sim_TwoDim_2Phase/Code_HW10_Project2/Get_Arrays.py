...
#%% Define Functions
def get_arrays(Num, PVT, Res,petro, BC, P, Sw, Sw_hyst):
    '''
    This function computes T, A, Act, Q, Qo, Qw, J, Jw, Jo, G
    in matrix or vector form to solve the diffusivity equation
    ...
    Pcow = lil_matrix((Num.N,1))
    dPcow = lil_matrix((Num.N,1))
    
    #Setting up the arrays Q, G
    Q = lil_matrix((Num.N,1))   # total rates vector
    Qo = lil_matrix((Num.N,1))  # Oil rates vector
    Qw = lil_matrix((Num.N,1))  # water rates vector
    G= lil_matrix((Num.N,1))    # Gravity vector
    ...
    for i in range(Num.N):
        Pc,dPc=CapillaryPressure(petro, Sw[i,0], Sw_hyst[i,0])
        Pcow[i,0]=Pc
        dPcow[i,0]=dPc
        Beta_o, Beta_w, A1, ct, C1, C2, C3 = Block_Properties(i, Sw, Res, PVT, petro, Num, dPcow[i,0])
        A[i,i]=A1
        Act[i,i]=A[i,i]*ct
    ...
    return T,Tw,To, A, Act, Q, J, Jw, Jo, G, BetaW, Jw, Jo,  Qw, Qo, G1, Pcow
