...
#%% Define functions  
def get_arrays_SS(Num, PVT, Res,PP, BC, P, Sw):
    ...
    for i in range(Num.N):
        Beta_o, Beta_w, A1, ct, C1, C2, C3=Block_Properties(i, Sw, Res, PVT, PP, Num)
        A[i,i]=A1
        Act[i,i]=A[i,i]*ct
        C[2*i,2*i]=C1
        C[2*i+1,2*i+1]=-A1; 
        ...
        if (i+1)%Num.Nx!=0: #Not in the right boundary
            T_interblock_w, T_interblock_o=T_interblock(i,i+1, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'x', P, Sw, PP, PVT)
        ...
        if i//Num.Nx>0:
            T_interblock_w, T_interblock_o=T_interblock(i,i-Num.Nx, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'y', P, Sw, PP, PVT)
        ...
        if (i+1)%Num.Nx==1:
            if 'Neumann' in BC.type[0]:
                ...
                J[i,i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'x', P, Sw, PP, PVT)
                Q[i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'x', P, Sw, PP, PVT)*BC.value[0]
        ...
        if (i+1)%Num.Nx==0:
            if 'Neumann' in BC.type[1]:
                ...
                J[i,i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'x', P, Sw, PP, PVT)
                Q[i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'x', P, Sw, PP, PVT)*BC.value[1]
        ...
        if i//Num.Nx==0:
            if len(BC.type)>2:
                if 'Neumann' in BC.type[2]:
                    ...
                    J[i,i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'y', P, Sw, PP, PVT.muo)
                    Q[i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'y', P, Sw, PP, PVT)*BC.value[2]
            else:
                None
        ...
        if i//Num.Nx==Num.Ny-1:
            if len(BC.type)>2:
                if 'Neumann' in BC.type[3]:
                    ...
                    J[i,i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'y', P, Sw, PP, PVT)
                    Q[i]=2*T_interblock(i,i, Num.dz, Num.dx, Num.dy, Res.kx, Res.ky,  'y', P, Sw, PP, PVT)*BC.value[3]
            else:
                None
    ...
    G=csr_matrix(G); Act=csr_matrix(Act); A=csr_matrix(A);
    T=csr_matrix(T); Tw=csr_matrix(Tw); To=csr_matrix(To);  C=csr_matrix(C)
