...
def T_interblock(i,j, dz, dx, dy, kx, ky,  direction, P, Sw, petro, PVT, Res):
    ...
    # The direction of the potentails has been specified (This is a trick because at the beggining the potentials are equal)
    # INITPOT = np.asarray([4,8,3,5,9,2,6,7,1,])*1e-5
    POT_i  = P[i,0]  -  PVT.rhoo/ 144.0 * (Res.D[i] - Res.woc)
    POT_j  = P[j,0]  - PVT.rhoo/ 144.0 * (Res.D[j] - Res.woc)
    #print(POT_i, POT_j)
    
    #UPWINDING fluid properties Eq. 9.17b
    if POT_i >= POT_j:
        krw,kro = BCM_RelativePermeability(Sw[i,0], petro)
        #For this assignment -->  properties are  known and NOT time depedant
        Bw  = PVT.Bw 
        Bo  = PVT.Bo
        # Bg = PVT.Bg  
        muw = PVT.muw 
