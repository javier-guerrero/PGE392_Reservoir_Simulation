...
if isinstance(Res.D, str):
    Depth = np.loadtxt("depth3x3.txt", comments="#", unpack=False)
    Res.D = Depth
if isinstance(Res.kx, str):
    Permeability = np.loadtxt("perm3x3.txt", comments="#", unpack=False)
    Res.kx = Permeability
if isinstance(Res.phi, str):
    Porosity = np.loadtxt("poro3x3.txt", comments="#", unpack=False)
    Res.phi = Porosity
